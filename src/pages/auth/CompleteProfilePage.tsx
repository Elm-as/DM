...
const CompleteProfilePage: React.FC = () => {
  ...
  const onSubmit = async (data: ProfileFormData) => {
    setIsLoading(true);
    try {
      // If the user was redirected here after email confirmation, the session
      // may be embedded in the URL. Try to extract and persist it before
      // performing any authenticated operations.
      try {
        // typed helper to avoid 'any' lint errors
        type SessionLike = { data?: { session?: { user?: { id: string; email?: string } } } };
        type AuthWithGetter = { getSessionFromUrl?: (opts?: { storeSession?: boolean }) => Promise<SessionLike> } & typeof supabase.auth;
        const authWithGetter = supabase.auth as AuthWithGetter;
        if (typeof authWithGetter.getSessionFromUrl === 'function') {
          await authWithGetter.getSessionFromUrl({ storeSession: true });
        }
      } catch (e) {
        console.warn('getSessionFromUrl unavailable or failed:', e);
      }
