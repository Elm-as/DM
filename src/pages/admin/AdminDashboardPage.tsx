...
const AdminDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      try {
        ...
        const users: UserRow[] = (data || []).map((u: { id: string; email: string; full_name: string; role: string; banned: boolean; user_credits?: { credits?: number } | null; }) => ({
          id: u.id,
          email: u.email,
  ...
  const updateCredits = async (userId: string, newCredits: number) => {
    await supabase.from('user_credits').update({ credits: newCredits }).eq('user_id', userId);
