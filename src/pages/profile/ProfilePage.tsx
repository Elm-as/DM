...
type FavoriteRow = { listing_id: string | null };
...
const ProfilePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      ...
      try {
        const [listingsRes, reviewsRes, favoritesRes] = await Promise.all([
          supabase
            .from('listings')
            .select('*')
            .eq('user_id', user.id)
        ...
        if (favoritesRes.error) {
          ...
          const favoriteIds = ((favoritesRes.data as FavoriteRow[] | null) || [])
          ...
          if (favoriteIds.length === 0) {
            ...
            const { data: favoriteListings, error: favoritesListingsError } = await supabase
              .from('listings')
              .select('*')
              .in('id', favoriteIds);

            if (favoritesListingsError) {
              console.error('Error fetching favorite listings:', favoritesListingsError);
              errors.push('favoris');
            } else {
              const favoritesMap = new Map((favoriteListings ?? []).map((listing) => [listing.id, listing]));
              const orderedFavorites = favoriteIds
                .map((id) => favoritesMap.get(id))
                .filter((listing): listing is Listing => Boolean(listing));
              setUserFavorites(orderedFavorites);
            }
          }
        }

        if (errors.length && isMounted) {
          toast.error(`Erreur lors du chargement: ${errors.join(', ')}`);
        }
      } catch (error) {
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg sm:rounded-xl lg:rounded-2xl shadow-md lg:shadow-lg p-4 sm:p-6">
              <div className="flex flex-col items-center text-center mb-4 sm:mb-6">
                ...
                <h1 className="text-lg sm:text-xl lg:text-2xl font-bold">{userProfile.full_name || 'Utilisateur'}</h1>
              ...
              <div className="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-sm sm:text-base">{userProfile.district || 'Non renseigné'}</span>
                </div>
              </div>
