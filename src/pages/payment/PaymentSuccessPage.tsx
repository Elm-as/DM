...
const PaymentSuccessPage: React.FC = () => {
  ...
  useEffect(() => {
    const updateTransactionStatus = async () => {
      try {
        ...
        // Si retour FusionPay avec token, interroger le statut
        if (paymentType === 'fusionpay' && token) {
          try {
            let statusUrl = '/api/fusionpay-check-status';
            if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
              statusUrl = '/.netlify/functions/fusionpay-check-status';
            }
            const r = await fetch(`${statusUrl}?token=${encodeURIComponent(token)}`);
            const d = await r.json();
            const paid = (d?.statut === true && d?.data?.statut === 'paid') || d?.data?.statut === 'paid';
            if (paid) {
              setMessage('Votre paiement a été confirmé.');
            } else if (d?.data?.statut === 'pending') {
              setMessage('Paiement en cours de traitement...');
            } else if (d?.data?.statut === 'failed' || d?.data?.statut === 'failure' || d?.data?.statut === 'no paid') {
              setMessage("Le paiement n'a pas abouti.");
            }
          } catch {
            // no-op, fallback aux vérifications locales
          }
        }
  ...
  }, [listingId, paymentType, userId, token]);
