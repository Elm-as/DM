...
const ListingPaymentForm: React.FC<ListingPaymentFormProps> = ({ listingId, userEmail, userPhone, userFullName, onSuccess }) => {
  ...
  const startFusionPay = async () => {
    if (!fullName || !phone) {
      toast.error('Nom complet et téléphone sont requis');
      return;
    }
    setIsStartingPayment(true);
    try {
      let apiUrl = '/api/fusionpay-create-payment';
      if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        apiUrl = '/.netlify/functions/fusionpay-create-payment';
      }
      const res = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
          ...
          userId: undefined, // le webhook utilisera personal_Info.listingId, userId peut être ajouté si disponible côté appelant
          fullName,
          phone,
          amount: 200
        })
      ...
      if (!res.ok) throw new Error(data.error || 'Erreur démarrage paiement');
      if (data?.url) {
      ...
      const message = e instanceof Error ? e.message : 'Erreur démarrage paiement';
      toast.error(message);
    } finally {
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-2xl shadow-2xl p-8 border border-primary-100 animate-fade-in">
      ...
      <div className="space-y-3 mb-8">
        <button
          type="button"
          onClick={startFusionPay}
          className="w-full py-3 text-lg font-semibold rounded-xl shadow-card border border-green-200 bg-green-600 hover:bg-green-700 text-white transition disabled:opacity-60 disabled:cursor-not-allowed"
          disabled={isStartingPayment}
        >
          {isStartingPayment ? 'Démarrage du paiement…' : 'Payer maintenant (Mobile Money)'}
        </button>
        <p className="text-xs text-center text-grey-500">Vous serez redirigé vers la page de paiement sécurisée.</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        >
          {isSubmitting ? (
            ...
            <>
              <span role="img" aria-label="envoyer">📤</span> Envoyer une preuve manuelle
            </>
          )}
        </button>
        {isSubmitting && (
