...
export const SupabaseProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateUserProfile = async (profile: Partial<Database['public']['Tables']['users']['Update']>) => {
    ...
    try {
      ...
      if (!error) {
        // Optimistic local update to avoid UI lag on redirects/CTAs
        setUserProfile((prev) => {
          if (!prev) {
            const optimistic: Database['public']['Tables']['users']['Row'] = {
              id: user.id,
              email: user.email || '',
          ...
          const merged: Database['public']['Tables']['users']['Row'] = {
            ...prev,
            ...(profile as Partial<Database['public']['Tables']['users']['Row']>),
          };
          return merged;
        });
        // Then refresh from DB to stay authoritative
        await fetchUserProfile(user.id);
      }
